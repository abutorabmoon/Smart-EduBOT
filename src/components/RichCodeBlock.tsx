...
const highlightCppCode = (code: string): JSX.Element[] => {
  ...
  return lines.map((line, lineIndex) => {
    ...
    return (
      <div key={lineIndex} className="flex group hover:bg-blue-800/30 transition-colors duration-150">
        {/* Line number */}
        <span className="text-blue-500 text-sm mr-6 select-none min-w-[3rem] text-right font-mono leading-6 pt-0.5">
          {lineIndex + 1}
        </span>
        {/* Code content */}
...
const tokenizeLine = (line: string): Token[] => {
  ...
  const tokenPatterns = [
    ...
    { regex: /#\w+/g, type: 'preprocessor', className: 'text-purple-400 font-medium' },
    // Numbers
    { regex: /\b\d+(\.\d+)?([eE][+-]?\d+)?[fFlL]?\b/g, type: 'number', className: 'text-amber-400' },
    ...
    // Brackets and braces
    { regex: /[(){}\[\]]/g, type: 'punctuation', className: 'text-yellow-400' },
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3">
              ...
              {language && (
                <span className="text-xs bg-blue-600 px-2.5 py-1 rounded-md text-white font-medium border border-blue-600/50">
          ...
          {showCopyButton && (
            <button
              onClick={handleCopy}
              className="text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 rounded-lg p-2 transition-all duration-200 bg-blue-600"
